...
# Get dataset information and count matrix
GSE156063 <- getGEO("GSE156063", destdir = "../Data/GSE156063/")
GSE156063_counts <- read.csv('../Data/GSE156063/GSE156063_swab_gene_counts.csv',row.names=1)
...
GSE156063_expressionMatrix <- GSE156063_expressionMatrix[!is.na(rownames(GSE156063_expressionMatrix)),]

# Remove GSE156063 outliers
GSE156063_outliers <- RNAseqQA(GSE156063_expressionMatrix,toPNG = FALSE, toPDF = FALSE,toRemoval = TRUE)

# Get severity labels (only interesting labels)
GSE156063_severity <- read_excel('../Data/GSE156063/Patient_class_inpatient_outpatient_MickKammetal.xlsx')
...
GSE156063_severity <- GSE156063_severity[-which(GSE156063_severity$Patient_class=='Emergency'),]
GSE156063_severity$Patient_class[which(GSE156063_severity$ICU=='ICU')] <- 'ICU'
print(table(GSE156063_severity$Patient_class)) # Print raw

# Remove outliers
GSE156063_expressionMatrix <- GSE156063_outliers$matrix
GSE156063_severity <- GSE156063_severity[! GSE156063_severity$CZB_ID %in% GSE156063_outliers$outliers,]
print(table(GSE156063_severity$Patient_class)) # Print without outliers

# Pick only interesting severity labels 
...
# Get dataset information and expression matrix (already processed)
GSE162835 <- getGEO("GSE162835", destdir = '../Data/GSE162835/')
GSE162835_expressionMatrix <- as.matrix(read_excel('../Data/GSE162835/GSE162835_COVID_GEO_processed.xlsx'))
rownames <- GSE162835_expressionMatrix[,1]
rownames(GSE162835_expressionMatrix) <- rownames
GSE162835_expressionMatrix <- GSE162835_expressionMatrix[,2:ncol(GSE162835_expressionMatrix)]
GSE162835_expressionMatrix <- apply(GSE162835_expressionMatrix,2,as.numeric)
rownames(GSE162835_expressionMatrix) <- rownames

# Get severity labels
GSE162835_sup <- as.data.frame(read_excel('../Data/GSE162835/sup_info.xlsx'))
...
print(table(GSE162835_labels_severity)) # Print raw
...
GSE152075 <- getGEO("GSE152075", destdir = '../Data/GSE152075/')
...
#Get counts
GSE152075_counts <- as.matrix(read.table('../Data/GSE152075/GSE152075_raw_counts_GEO.txt', header =TRUE))
...
GSE152075_expressionMatrix <- GSE152075_expressionMatrix[!is.na(rownames(GSE152075_expressionMatrix)),]

#Get severity labels
GSE152075_severity <- read.csv('../Data/GSE152075/2021-03-19_Rojas.csv')
...
GSE152075_severity <-  GSE152075_severity[-which(GSE152075_severity$alt_name%in%GSE152075_outliers$outliers),]
...
GSE152075_expressionMatrix_severity <- GSE152075_expressionMatrix[,which(colnames(GSE152075_expressionMatrix)%in%GSE152075_severity$alt_name)]
...
save(list = c("expressionMatrix", "labels"), file = "../Data/joined_datasets.Rdata")